...
1. 아래 링크를 클릭합니다 (GitHub 로그인 필요, 무료 월 60시간).
   **https://codespaces.new/yooseong1478-ui/yoosung?quickstart=1**
2. "Create codespace" 를 누르고 2~5분 기다리면 설치가 자동으로 끝나고 **대시보드 탭이 자동으로 열립니다.**
   (안 열리면 화면 아래 "포트(PORTS)" 탭에서 8787 옆 지구본 아이콘을 누르세요)
3. 대시보드에서 순서대로:
   - **Claude 연결** → "Claude 로그인 시작" → 링크 열어 승인 → 코드 붙여넣기 (구독 계정, 한 번만)
   - **네이버 로그인** → "네이버 로그인 창 열기" → "로그인 화면 보기" 로 원격 화면을 열어 로그인 (한 번만, 세션 저장)
     · 해외 접속이라 "새로운 기기" 인증 문자가 오면 그 화면에서 입력
   - **글감 찾기** → **글 쓰기** (또는 "준비된 글 불러오기") → **임시저장 / 발행**
4. 다 쓰면 그냥 탭을 닫으면 됩니다. 30분 뒤 자동으로 멈추고, 다시 링크를 열면 저장된 로그인 그대로 이어집니다.
   (30일 동안 안 쓰면 Codespace 가 삭제되어 로그인만 다시 하면 됩니다)

원격 화면 비밀번호를 물으면 `vscode` 입니다. 문제가 생기면 대시보드 맨 아래 "진행 로그" 내용을 복사해 주세요.
